...
class FrequencyAdaptor(nn.Module):
    def __init__(self, hidden_dim, feat_dim, frequency_tune, embedding_tune, adaptor):
        """
        Args:
        """
        ...
        if self.frequency_tune:
            ...
            # self.mlp_mask = nn.Linear(feat_dim*2, 1)
            # self.weight = nn.Parameter(torch.zeros(h, w // 2 + 1, 1, 2, dtype=torch.float32))
            # self.weight = nn.Parameter(torch.zeros((h * w) // 2 + 1, 1, 2, dtype=torch.float32))
            self.sigmoid = nn.Sigmoid()

        if self.frequency_tune and self.embedding_tune:
            if adaptor:
                # self.down_frequency = nn.Sequential(
                #     nn.Linear(self.feat_dim, self.hidden_dim),
                #     nn.GELU(),
                # )
                # self.down_embedding = nn.Sequential(
                #     nn.Linear(self.feat_dim, self.hidden_dim),
                #     nn.GELU(),
                # )
    ...
    def get_frequency(self, prompt):
        # B, N, C = prompt.shape
        # h = w = int(math.sqrt(N))
        # prompt = prompt.view(B, h, w, C)
        # ffted = torch.fft.rfft2(prompt, dim=(1, 2), norm='ortho')
        ...
        # real_mask = imag_mask = self.sigmoid(self.mlp_mask(torch.cat([ffted.real, ffted.imag], dim=-1)))
        # mask = torch.complex(real_mask, imag_mask)
        # ffted = ffted * mask
        # weight = self.sigmoid(torch.view_as_complex(self.weight))
        # weight = torch.view_as_complex(self.weight)
        # ffted = ffted * weight

        # freq_part = torch.fft.irfft2(ffted, s=(h, w), dim=(1, 2), norm='ortho')
        # freq_part = freq_part.reshape(B, N, C)
        #
        ...
        # self.mask = torch.complex(real_mask, imag_mask)
        # self.mask = torch.fft.irfftn(self.mask, s=ifft_shape_slice, dim=fft_dim, norm='ortho')
    ...
    def forward(self, prompt):
        ...
        if self.embedding_tune and self.frequency_tune:
            if self.adaptor:
                # prompt_frequency = self.down_frequency(self.get_frequency(prompt))
                # prompt_embedding = self.down_embedding(self.get_embedding(prompt))
                ...
                # prompt_frequency = self.mlp_frequency(prompt_frequency)
                # prompt_embedding = self.mlp_embedding(prompt_embedding)
                prompt = self.up(prompt_frequency + prompt_embedding)
                # prompt = self.up(prompt_frequency) + self.up(prompt_embedding)
                # prompt = self.up(torch.cat([prompt_frequency, prompt_embedding], dim=-1))
                return prompt
            else:
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.config and self.config.ffn_option == 'parallel':
