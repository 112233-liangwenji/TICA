...
# from train import to_get_params
class Averager():
...
def compute_pixel_accuracy(
    ...
    # return torch.tensor(binary_pred_mask == gt_mask).to(torch.float32).mean(dim=(0, 1))
    return torch.eq(binary_pred_mask, gt_mask).float().mean().item()
    # return torch.tensor(binary_pred_mask == gt_mask).to(torch.float32).mean(dim=(-1, -2)).cpu()
...
def calc_uda(y_pred, y_true):
    ...
    with torch.no_grad():
        ...
        for i in range(batchsize):
            ...
            # metric_FM.step(pred=pred, gt=true)
            # metric_IoU.step(pred=pred, gt=true)
            # metric_ACC.step(pred=pred, gt=true)

            # acc = accuracy(pred, true)
            # iou = IoU(pred, true)

            pred_bi = torch.from_numpy(pred)
            # print('pred', pred)

            binary_pred = (pred_bi > 0.5).float().squeeze()
            # print('binary_pred', binary_pred)

            gt_mask = torch.from_numpy(true)
            gt_bi = normalize_tensor(gt_mask)
            # print('true', true)
            # print('gt_mask', gt_bi)

            acc = compute_pixel_accuracy(binary_pred, gt_bi)
            iou = compute_iou(binary_pred, gt_bi)

            # acc = compute_pixel_accuracy(pred, true)
            # iou = compute_iou(pred, true)

            # acc = accuracy(pred, true)
            # iou  = IoU(pred, true)

    return f_max / batchsize, acc, iou
